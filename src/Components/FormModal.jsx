...
const FormModal = ({ isOpen, onClose, onSubmit, defaultValues }) => {
  ...
  useEffect(() => {
    if (defaultValues) {
      setFormData({
        ...
        username: defaultValues.username, // Pre-filled but non-editable
      ...
      setFormData((prev) => ({ ...prev, username: `USER-${prev.name}` }));  // Initialize username as non-editable
    }
  }, [defaultValues]);
  ...
  const validateForm = () => {
    ...
    if (formData.companyName && formData.companyName.length < 3) {
      newErrors.companyName = 'Company name must be at least 3 characters if provided.';
    }
  ...
  return (
    <div style={{
      ...
      <div ref={modalRef} style={{
        ...
        <h2 style={{ color: '#1F2937' }}>{defaultValues ? 'Edit User' : 'Add User'}</h2>
        <form>
        ...
        <button onClick={handleSubmit} style={{ backgroundColor: '#4CAF50', color: 'white', padding: '10px', marginRight: '10px' }}>
          {defaultValues ? 'Save' : 'Add'}
        </button>
        <button onClick={onClose} style={{ backgroundColor: '#f44336', color: 'white', padding: '10px' }}>
          Cancel
        </button>
      </div>
    </div>
